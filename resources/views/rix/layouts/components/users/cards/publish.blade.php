<div class="post-publish w-100">
    @if(!isset($user) || Auth::user()->user_id != $user->user_id)
        ...
        <div class="form-group m-3 d-inline-block">
            <div class="custom-control custom-radio">
                <input type="radio" id="role3" name="role" class="custom-control-input" value="user"
                       @isset($user) {{$user->role === 'user' ? 'checked' : null}} @else checked @endisset >
                <label class="custom-control-label" for="role3">Kullanıcı</label>
            </div>
        </div>
    @endif
    <div class="invalid-feedback" data-name="role"></div>
