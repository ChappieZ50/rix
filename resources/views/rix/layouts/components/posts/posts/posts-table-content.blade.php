@foreach($posts as $post)
    ...
    <tr>
        ...
        <td>{{$post->title}} {!! $post->status == 'closed' && !Request::get('type') ? '<b>— Gönderilmemiş </b>' : null !!}
        </td>
        ...
        <td>
            <div class="btn-group dropleft">
                ...
                <div class="dropdown-menu dropleft">
                    <a class="dropdown-item has-icon"><i class="far fa-edit"></i> Düzenle</a>
                    @if(Request::get('type') == 'trash')
                        <a class="dropdown-item has-icon" href="javascript:;"><i class="fas fa-trash-restore-alt"></i>Geri Al</a>
                        <a class="dropdown-item has-icon" href="javascript:;" id="singlePermanentlyDelete" style="color:red;" data-id="{{$post->post_id}}" data-status="{{$post->status}}"><i class="far fa-trash-alt"></i>Kalıcı Olarak Sil</a>
                    @else
